...
| `--dispersion` | override the fitted α with a fixed value (finite, ≥ 0; 0 is Poisson) applied to every locus; bypasses `--fit-type` and `--min-signal` entirely and sets `dispersion_fit = override`. Useful for sensitivity analyses (e.g. re-run at 0.05 and 0.10 to see how much calls depend on α) |
